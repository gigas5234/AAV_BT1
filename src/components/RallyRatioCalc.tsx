...
/** The two march recipes. Numbers are raw troop counts, so they paste into the game as-is. */
const RALLY: Record<Kind, number> = { inf: 10_000, cav: 10_000, arc: 80_000 } // 10/10/80 = 100K
const JOIN: Record<Kind, number> = { inf: 8_000, cav: 8_000, arc: 56_000 } //  8/8/56  =  72K
...
export default function RallyRatioCalc({ mode }: { mode: R856Mode }) {
  ...
  // recipe for march i (0-based) under the current mode
  const recipe = (i: number) => (mode === 'host' && i === 0 ? RALLY : JOIN)

  const need = useMemo(() => {
    const acc: Record<Kind, number> = { inf: 0, cav: 0, arc: 0 }
    for (let i = 0; i < marches; i++) for (const k of KINDS) acc[k] += recipe(i)[k]
    return acc
  }, [marches, mode])
  ...
  const covered = useMemo(() => {
    ...
    return marches
  }, [marches, mode, owned.inf, owned.cav, owned.arc])

  const joinCount = mode === 'host' ? Math.max(0, marches - 1) : marches
    ...
    <div className="space-y-3">
      ...
      <section className="rounded-2xl border-2 p-3.5" style={{ borderColor: `${accent}66`, background: `${accent}12` }}>
        ...
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          {mode === 'host' && <Chip label={t('r856.chipRally')} value="10 / 10 / 80" color={accent} />}
          <Chip label={t('r856.chipJoin')} value="8 / 8 / 56" color={accent} />
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
        <div className="flex items-center justify-between text-sm">
          <span className="text-slate-300">{t('r856.marches')}</span>
          <div className="flex items-center gap-2">
        ...
        <div className="mt-3 border-t border-white/5 pt-3">
            ...
            {KINDS.map((k) => (
              <label key={k} className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onChange={(e) => setOwned((o) => ({ ...o, [k]: Math.max(0, Math.round(Number(e.target.value) || 0)) }))}
          ...
          <p className="mt-1.5 text-[11px] text-slate-500">{t('r856.ownedHint')}</p>
        </div>
      </section>
              ...
              {anyShort ? (
                <>
                  ...
                  <p className="mt-2.5 rounded-lg bg-black/25 px-3 py-2 text-[12px] leading-relaxed text-red-100">
                    {t('r856.covered', { n: String(covered), total: String(marches) })}
                  </p>
                </>
              ) : (
                <>
                  <p className="text-[13px] font-semibold leading-relaxed text-emerald-200">
                    {t('r856.enoughBody', { n: String(marches) })}
                  </p>
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[12px]">
                    {KINDS.map((k) => (
                      <span key={k} className="flex items-center gap-1.5">
          ...
          <section className="overflow-hidden rounded-2xl border border-white/10">
            <h3 className="bg-white/[0.06] px-3.5 py-2.5 text-[13px] font-semibold text-white">
              ...
              <span className="ml-1.5 text-[11px] font-normal text-slate-400">
                {mode === 'host' ? t('r856.planHost', { n: String(joinCount) }) : t('r856.planJoin', { n: String(joinCount) })}
              </span>
            </h3>
            <div className="divide-y divide-white/5">
