...
const STR: Record<string, Entry> = {
  ...
  'gov.enemyTitle': { en: 'Enemy scouting', ko: '적 정보' },
  'gov.enemyNote': { en: 'Scouted from the opposing kingdom. Tap any shot to enlarge.', ko: '상대 왕국을 정찰한 화면입니다. 사진을 누르면 크게 볼 수 있습니다.' },
  'gov.enemyPlayers': { en: 'Key players', ko: '주요 인원' },
  'gov.enemyRanking': { en: 'Power ranking', ko: '전투력 순위' },
  'gov.enemyHeroes': { en: 'Hero setup', ko: '영웅 구성' },
  'gov.enemyClose': { en: 'Close', ko: '닫기' },
  ...
  // ── 8/8/56 rally calculator (its own two tabs, separate from the role calculator) ──
  'r856.hostTab': { en: '8/8/56 host', ko: '8/8/56 집결자' },
  'r856.joinTab': { en: '8/8/56 joiner', ko: '8/8/56 참여자' },
  'r856.hostTitle': { en: '8/8/56 rally — host', ko: '8/8/56 렐리 · 집결자' },
  'r856.joinTitle': { en: '8/8/56 rally — joiner', ko: '8/8/56 렐리 · 참여자' },
  'r856.hostDesc': {
    en: 'Open your own rally at 10/10/80 — that fills your march. It costs you no join slot: you still send an 8/8/56 join to every other rally on top of it.',
    ko: '자기 집결은 10/10/80으로 엽니다. 이건 행군을 꽉 채웁니다. 집결을 열어도 참여 슬롯은 그대로라, 다른 렐리에는 8/8/56 참여 행군을 그대로 다 보냅니다.',
  },
  'r856.joinDesc': {
    en: 'You do not open a rally. Every march you send is 8/8/56 — nothing else.',
    ko: '집결을 열지 않습니다. 보내는 행군은 전부 8/8/56 렐리만 사용합니다.',
  },
  'r856.fill72': {
    en: '8 + 8 + 56 = 72, not 100. A join march uses only 72% of your capacity — {join} out of {cap}. The rest stays home.',
    ko: '8 + 8 + 56 = 72입니다. 100이 아닙니다. 참여 행군은 수용량의 72%만 씁니다 — {cap} 중 {join}. 나머지는 보내지 않습니다.',
  },
  ...
  'r856.how1Title': { en: '1. Max troop size — every number is a % of it', ko: '1. 최대 병력 규모 — 모든 수치는 이 값의 %입니다' },
  'r856.how1Body': {
    en: 'Enter how many troops one march of yours holds — 100K by default, 130 if it holds 130K. The numbers are percentages of it, so 10/10/80 fills the march (100%) while 8/8/56 only fills 72%. At 130K a join sends 10,400 / 10,400 / 72,800 = 93,600, not 130,000.',
    ko: '내 행군 1개에 들어가는 병력을 입력하세요. 기본 100K이고, 130K까지 들어가면 130을 넣습니다. 수치는 이 값의 퍼센트라서 10/10/80은 행군을 꽉 채우지만(100%) 8/8/56은 72%만 채웁니다. 130K면 참여 행군은 10,400 / 10,400 / 72,800 = 93,600이지 130,000이 아닙니다.',
  },
  'r856.how2Title': { en: '2. Join cap — use it when the rally limits you', ko: '2. 참여 제한 — 렐리가 병력을 제한할 때' },
  'r856.how2Body': {
    en: 'Turn it on when each join is capped, e.g. 80K. If the 8/8/56 march is already under the cap nothing changes; if it is over, it is trimmed down to the cap with the 8:8:56 shape kept. Your own rally is never capped. Leave it off when there is no limit.',
    ko: '참여 병력이 제한될 때(예: 80K) 켜세요. 8/8/56 행군이 이미 제한보다 작으면 그대로고, 넘으면 8:8:56 모양을 유지한 채 제한 크기까지 줄입니다. 자기 집결은 제한을 받지 않습니다. 제한이 없으면 꺼두세요.',
  },
  'r856.capacity': { en: 'Max troop size', ko: '최대 병력 규모' },
  'r856.capacityHint': {
    en: 'One march at full size. Every number below is a percentage of this.',
    ko: '행군 1개의 최대 병력. 아래 수치는 모두 이 값의 퍼센트입니다.',
  },
  'r856.limit': { en: 'Join cap', ko: '참여 제한' },
  'r856.limitHint': {
    en: 'Caps the 8/8/56 marches only — your own rally still goes out at full size.',
    ko: '8/8/56 행군에만 적용됩니다. 자기 집결은 최대 규모 그대로 나갑니다.',
  },
  'r856.limitNoop': {
    en: 'The 8/8/56 march is already under this cap, so nothing is trimmed.',
    ko: '8/8/56 행군이 이미 이 제한보다 작아서 줄어드는 것이 없습니다.',
  },
  ...
  'r856.planHost': { en: 'your rally + {n} × 8/8/56', ko: '자기 집결 + 8/8/56 × {n}' },
  'r856.planJoin': { en: '{n} × 8/8/56', ko: '8/8/56 × {n}' },
  'r856.rowRally': { en: 'Your rally · 10/10/80', ko: '자기 집결 · 10/10/80' },
  'r856.rowJoin': { en: 'Join · 8/8/56', ko: '참여 · 8/8/56' },
